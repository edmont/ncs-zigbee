...
{
	/*
	 * Load the persisted protocol before claiming the radio. If this
	 * device was already commissioned via Matter, a previous boot stored
	 ...
	 */
	int init_ret = protocol_state_init();
	__ASSERT(init_ret == 0, "Failed to initialise protocol state persistence: %d", init_ret);
	ARG_UNUSED(init_ret);
...
/* Dormant (delay = K_TICKS_FOREVER); started explicitly from the public entry. */
...
{
	if (cb == NULL || cb->matter_start == NULL || cb->zigbee_start == NULL) {
		return -EINVAL;
	}
