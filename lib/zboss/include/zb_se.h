...
typedef struct zse_commissioning_ctx_s
{
  ...
  void (*switch_to_bdb_commissioning)(zb_bdb_commissioning_step_t);
#endif
...
void se_commissioning_signal(zse_commissioning_signal_t sig, zb_bufid_t param);
#endif /* ZB_ENABLE_SE_MIN_CONFIG */

#ifdef ZB_SE_COMMISSIONING
void se_initiate_commissioning(zb_bufid_t param);
void se_commissioning_machine(zb_cb_param_t param);
...
zb_bool_t zb_se_service_discovery_match_desc_resp_handle(zb_bufid_t param);
zb_bool_t zb_se_service_discovery_read_commodity_attr_handle(zb_bufid_t param);
void zb_se_service_discovery_reset_dev(void);
void zb_se_service_discovery_send_start_signal_delayed(zb_cb_param_t param);
void zb_se_service_discovery_send_start_signal(zb_cb_param_t param);
zb_bool_t zb_se_service_discovery_block_zcl_cmd(zb_uint16_t short_addr, zb_uint8_t ep, zb_uint8_t zcl_tsn);

#ifdef ZB_ENABLE_TIME_SYNC
zb_bool_t zb_se_service_discovery_read_time_attrs_handle(zb_bufid_t param);
#endif
...
zb_uint8_t zb_zdo_init_ieee_addr_req_with_cb(zb_bufid_t param, zb_uint16_t nwk_addr, zb_callback_t cb);


#if defined(ZB_SE_ENABLE_STEADY_STATE_PROCESSING)

zb_bool_t zb_se_steady_state_match_desc_resp_handle(zb_bufid_t param);
void zb_se_steady_state_start_periodic_tc_poll(void);
void zb_se_steady_state_stop_periodic_tc_poll(void);


#if defined(ZB_ZCL_SUPPORT_CLUSTER_PRICE)

zb_ret_t zb_se_steady_publish_price_handle(zb_bufid_t param);

#endif

zb_bool_t zb_se_steady_state_block_zcl_cmd(zb_uint16_t short_addr, zb_uint8_t ep, zb_uint8_t zcl_tsn);

zb_bool_t zb_se_steady_state_read_attr_handle(zb_bufid_t param);

#endif

#ifdef ZB_SE_COMMISSIONING
zb_bool_t se_is_high_freq_msg(zb_bufid_t param);
zb_bool_t se_cbke_exchange_schedule_alarm(zb_bufid_t param, zb_callback_t alarm);
#ifdef ZB_COORDINATOR_ROLE
void se_comm_device_left(zb_address_ieee_ref_t addr_ref);
#endif /* ZB_COORDINATOR_ROLE */
#endif /* ZB_SE_COMMISSIONING */

void se_send_key_est_match_desc(zb_cb_param_t param);
void se_send_key_est_match_desc_addr(zb_cb_param_t param);
...
void se_handle_link_key_indication(zb_cb_param_t param);
