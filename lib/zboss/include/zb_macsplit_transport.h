 ...
 */
typedef void (*recv_data_cb_t)(zb_bufid_t);
 ...
 * @return nothing
 */
void zb_macsplit_transport_send_data_with_type(zb_cb_param_t cb_param);
...
#if defined ZB_MACSPLIT_DEVICE
  zb_uint16_t msdu_handles[ZB_N_BUF_IDS];  /*!< store msdu_handle from host */
#else
  zb_callback_t confirm_cb[ZB_N_BUF_IDS]; /*!< store callbacks by host */
  zb_uint8_t    recv_cfm[ZB_RECV_CFM_NBYTES]; /*!< bitmask to store received confirm packets buf id */
#endif /* ZB_MACSPLIT_DEVICE */

#else /* ZB_CONFIGURABLE_MEM */

#if defined ZB_MACSPLIT_DEVICE
  zb_uint16_t* msdu_handles;
#else
...
typedef struct zb_macsplit_transport_context_s
{
  ...
  zb_bufid_t                       tx_win[ZB_MACSPLIT_MAX_PACKET_NUMBER]; /*!< packet ids in tx win */
...
void mac_sync_address_update(const zb_ieee_addr_t ieee_address, zb_uint16_t short_address, zb_bool_t lock);
