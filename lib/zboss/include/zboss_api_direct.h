 ...
 * @note The callback is being called from user space (not from ISR)
 */
typedef void (*zbd_zdd_simple_callback_t)(zb_cb_param_t conn_id);
 ...
 *
 * @param uuid128 Pointer to 16-bytes array containing desired UUID
 ...
 *
 * @param cb_param packed buffer id to establish trusted link and connection id between ZVD and ZDD
 */
void zbd_zvd_establish_trusted_link(zb_cb_param_t cb_param);
 ...
 * @param [in]  conn_id     Connection ID
 */
typedef void (*zbd_zvd_simple_callback_t) (zb_cb_param_t conn_id);
